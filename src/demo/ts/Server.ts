...
const fullContent = `<h1 id="mcetoc_1ipdv0ulf969"></h1>
  <div class="mce-toc">
...
const page = (editor1Value: string, editor2Value: string, editor3Value: string, editor4Value: string) => `
  ...
  <html lang="en">
    <head>
      ...
      <script src="/configs/full-config.js"></script>
      <script>
...
const configPath = path.normalize(path.join(__dirname, '..', 'js', 'configs'));
...
app.use('/configs', express.static(configPath));

app.get('/', (request, response) => {
  response.send(page(fullContent, '', '', ''));
});
